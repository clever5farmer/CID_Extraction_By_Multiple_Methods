...
def createAugmentedImages(augDir, featureRootDir, imgList, labelImgList, fileNameList):
    for augment in Augmentation:
        print(augment.value)
        if os.path.exists(os.path.join(augDir, 'images', augment.value)):
        #if augment == Augmentation.AFFINE or augment == Augmentation.ORIGINAL:
            continue
        imagesAug, segmapsAug = imageAugmentation(imgList, labelImgList, augment)
        print(np.shape(imagesAug))
        saveImages(imagesAug, fileNameList, os.path.join(augDir, 'images', augment.value))
        saveImages(segmapsAug, fileNameList, os.path.join(augDir, 'labels', augment.value))
...
def imageAugmentation(imgList, labelImgList, augType):
    ...
    elif augType == Augmentation.TRANSLATION20:
    ...
    elif augType == Augmentation.CUTOUT:
